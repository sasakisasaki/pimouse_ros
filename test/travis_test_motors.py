...
class MotorTest(unittest.TestCase):
    ...
    def test_put_cmd_vel(self):
        ...
        self.file_check("rtmotor_raw_r0",0,"right: don't stop after 1 sec")
        self.file_check("rtmotor_raw_l0",0,"left: don't stop after 1 sec")

    def test_on_off(self):
        off = rospy.ServiceProxy('/motor_off', Trigger)
        ret = off()
        self.assertEqual(ret.success, True, "motor off failed")
        self.assertEqual(ret.message, "OFF", "motor off: wrong message")
        with open("/dev/rtmotoren0","r") as f:
            data = f.readline()
            self.assertEqual(data,"0\n","wrong value in rtmotoren0 when motor off")

        on = rospy.ServiceProxy('/motor_on', Trigger)
        ret = on()
        self.assertEqual(ret.success, True, "motor on failed")
        self.assertEqual(ret.message, "ON", "motor on: wrong message")
        with open("/dev/rtmotoren0","r") as f:
            data = f.readline()
            self.assertEqual(data,"1\n","wrong value in rtmotoren0 when motor on")
